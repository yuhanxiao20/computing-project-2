...
rain_df <- df %>% 
  ...
  mutate(rainfall_mean = sample_mean(rainfall),
...
rain_df %>%
  ...
  labs(title = "Estimates of Mean Rainfall and the 95% Confidence Intervals for Each City", subtitle="key trends summary", caption="Yuhan Xiao", x = "year", y = "average rainfall") + 
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5, vjust = 1, size=15))
```